...
// 
// // Gauge coupling
// const double gsm = 2*MW*sqrt(GF*sqrt(2));
// // CKM Matrix
// const double Vud = 0.97427;
// const double Vus = 0.2253;
// const double Vub = 0.00351;
// const double Vcd = 0.2252;
// const double Vcs = 0.97344;
// const double Vcb = 0.0412;
// const double Vtd = 0.0;
// const double Vts = 0.0;
// const double Vtb = 1.0;


// SM parameters (as Stefano for validation)
#define MW 79.9571
...
// // SM parameters for SSM BSM
// #define MW 80.425
// #define GammaW 2.124

#define alphaEM 1 / 128
...
// const double GammaWprime = 12 * pow(gsm,2) * MWprime / (48*PI);
...
// const double energy = 13000;

// Acceptance cuts
const double eta_cut = 2.5;
const double pT_cut = 25;
const double MT_min = 40;
...
double jac (double &yreduced, double &Minv, double *x, Parameters *params)
{
...
//     // Linear sampling
//     Minv = (Minv_max - Minv_min) * x[1] + Minv_min;
//     djac *= Minv_max - Minv_min;
    
    // Log sampling
     Minv = Minv_min * pow(Minv_max / Minv_min, x[1]);
     djac *= Minv * log(Minv_max / Minv_min);
     
    
    // Acceptance cuts
    double z = pow(Minv,2)/pow(energy,2);
    double y = -0.5*log(z)*yreduced;
...
double udbar_funct (double *x, size_t dim, void *jj)
{
    ...
    double yreduced, Minv;
    double djac = jac(yreduced, Minv, x, params);
...
//     double propagator = 1 / (pow(pow(Minv,2) - pow(MW,2),2) + pow(GammaW,2) * pow(MW,2)); // Propagator (squared) single W    
    ...
    dsigma *= 2 * Minv; // Jacobian Minv^2 -> Minv
...
//     }
    ...
    text = "Wprpluseigenpoints[" + std::to_string(MWprime) + "] = {{";
    ...
    cout << "Computing W- eta spectrum" << endl;
    begin = std::chrono::steady_clock::now();
    text = "Wprminuseigenpoints[" + std::to_string(MWprime) + "] = {{";
