...
//     if ( (y < y_min) || (y > y_max and y_max != 0)) return 0;
    ...
    double dsigma = GeVtofb / (768 * pow(PI,2)) * pow(Minv,2) / 4 * pow(1 + costheta,2); // Born matrix element squared without propagator
//     double propagator = 1 / (pow(pow(Minv,2) - pow(MW,2),2) + pow(GammaW,2) * pow(MW,2)); // Propagator single W    
    double propagator = pow(gsm,4) * (1 / (pow(pow(Minv,2) - pow(MW,2),2) + pow(GammaW,2) * pow(MW,2))) + pow(gsmprime,4) * (1 / (pow(pow(Minv,2) - pow(MWprime,2),2) + pow(GammaWprime,2) * pow(MWprime,2))) + pow(gsm,2) * pow(gsmprime,2) * (2 * ((pow(Minv,2) - pow(MW,2)) * ((pow(Minv,2) - pow(MWprime,2))) + MW * GammaW * MWprime * GammaWprime) / (((pow(pow(Minv,2) - pow(MW,2),2) + pow(GammaW,2) * pow(MW,2))) * ((pow(pow(Minv,2) - pow(MWprime,2),2) + pow(GammaWprime,2) * pow(MWprime,2))))); // Propagator W + W' +Interf    
...
//     strcpy(filename, "_");
    ...
    cout << "GammaWprime/MWprime = " << GammaWprime/MWprime << endl;
